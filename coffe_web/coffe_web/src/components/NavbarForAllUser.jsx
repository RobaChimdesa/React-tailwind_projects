...
import MenuIcon from "@mui/icons-material/Menu";
import CloseIcon from "@mui/icons-material/Close";
import Signin from "./Signin"; // Assuming this is your login component
import { useNavigate } from "react-router-dom";

const NavbarForAllUser = () => {
  const [menu, setMenu] = useState(false);
  const [showLogin, setShowLogin] = useState(false); // State for showing the login form
  const navigate = useNavigate();
  ...
  const handleLoginClick = () => {
    setShowLogin(true); // Show the login form when the button is clicked
    navigate("/signin")
  };
  ...
  return (
    <div className="">
      <div className="flex flex-row justify-between p-5 px-5 lg:px-32 bg-slate-50 py-7 shadow-md">
        ...
        <div className="hidden lg:flex">
          <Button title="Sign in" onClick={handleLoginClick} />
        </div>
      ...
      {showLogin && (
        <div
          className={`fixed inset-0 flex items-start justify-center transition-transform duration-300 transform ${
            showLogin ? "translate-y-0" : "-translate-y-full"
          }`}
        >
          <div className="bg-white p-6 rounded shadow-md w-full max-w-md">
            <Signin onClose={handleCloseLogin} /> {/* Pass close handler */}
          </div>
        </div>
      )}
    </div>
